...
# high_load 초과 -> busy(performance), low_load 미만 -> idle(normal) 전환이 자연히 발생한다.
...
busy=$(logcount '"event":"policy.busy"')
idle=$(logcount '"event":"policy.idle"')

narrate "관측된 regime 전환 (policy.busy / policy.idle):"
docker logs "$CTRL" 2>&1 | grep -E '"event":"policy.(busy|idle)"' |
