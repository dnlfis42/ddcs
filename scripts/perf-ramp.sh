...
#
# 핵심 지표는 sweep tick 한 번의 소요 시간(us)이다. 한 tick은 명령 재전송, liveness 검사, 정책 평가를
# 한 스레드에서 처리하므로, 이 시간이 sweep 주기(기본 1s = 1,000,000us)에 근접하면 한 코어가 포화에 이른다.
# pending이 쌓이거나 evicted가 늘어도 Controller가 따라가지 못하고 있다고 봐야 한다.
#
...
#                                   Group 전체를 한 tick에 재명령하는 burst가 Group 크기만큼 커진다)
...
#       sweep_avg_us는 Controller가 tick당 실제로 일한 시간이라 호스트 경합에 오염되지 않는다.
...
if [ "$SINGLE" = "1" ]; then
    narrate "성능 램프(단일 Group burst): 레벨 = [$LEVELS] (총 Agent 수, 전부 zone_a), 레벨당 ${SOAK}s 측정"
else
...
# 예외는 sweep_max_cum 하나: 시작 후 누적 최대라 리셋이 없어 이전 레벨과 레벨 전환(접속 폭풍)을
...
narrate "해석"
info "표의 누적 지표(sweep_avg_us, in_msgs_s, rtt_ms, evicted)는 ${SOAK}초 측정 창에서 늘어난 양으로 계산한다."
info "sweep_max_cum만 시작부터의 최댓값이라, 레벨을 올릴 때 수백 대가 한꺼번에 접속하는 구간까지 포함한다."
info "sweep_avg_us가 sweep 주기(1초)에 근접하면 코어 하나로는 더 감당하지 못하며, pending이나 evicted가 0이 아닌 행도 같은 신호로 본다."
