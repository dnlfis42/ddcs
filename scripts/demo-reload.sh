#!/usr/bin/env bash
# DDCS 데모: policy 핫리로드. 운영자가 controller.json의 policy를 고치고 SIGHUP을 보내면
# 컨트롤러가 재시작 없이 새 정책을 다시 읽고, commanded belief를 비운 뒤 동작 중인 fleet을 재명령한다.
# 반면 malformed한 편집은 적용 전에 걸러 거부하고 옛 정책을 유지해 fleet을 지킨다.
# 관측: controller 로그의 policy.load / policy.load.fail(trigger=reload) + zone_a 모드 분포.
...
# wait_for와 단언에 쓸 술어다. 중첩 따옴표를 피하려고 함수로 감싼다.
...
# device들이 부팅 정책으로 한 번이라도 명령받아 commanded belief를 갖게 한다. 그래야 reload가
# belief를 비우고 다시 명령하는 과정을 단언이 실제로 짚는다.
...
# PHASE 1: 유효한 편집 후 SIGHUP을 보내 재적용과 재명령을 확인한다.
...
# 정책이 적용됐다면 계속 safe다. 3회(약 6s) 연속 safe면 정책이 강제한 것으로 본다.
za_persist=0
for _ in 1 2 3; do
