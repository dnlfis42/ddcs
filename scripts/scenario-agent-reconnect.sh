...
#
# 재시작한 agent가 재접속하면 device가 현재 목표 mode를 다시 명령받는지 검증한다.
...
# 대기 상한은 SOAK 노브와 분리해 고정한다. SOAK을 줄이면 여기가 거짓 FAIL했다.
wait_for "device가 정책 모드로 수렴(첫 명령 수신)" 60 dispatched_at_least "$DEV" 1 || true
soak 2 "명령 기억 정착"
