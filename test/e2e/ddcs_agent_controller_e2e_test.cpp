...
// 양쪽(Controller/Agent) 로그를 한 sink 에 모은다. logger singleton 이라 마지막 set_sink 가 이김 -
// 두 facade Config 에 같은 인스턴스를 명시 주입한다.
...
// 단일 스레드로 양 reactor 를 번갈아 펌프하며 pred 를 기다린다.
...
std::unique_ptr<ddcs::agent::Agent> make_agent(CaptureSink& sink, std::uint16_t port, std::uint8_t uuid_seed) {
...
TEST(AgentControllerE2eTest, AgentRegistersAndReachesActive) {
    ...
    bool const active = pump_until(*controller, *agent, [&] { return agent->session().state() == State::active; });

    ASSERT_TRUE(active);
    EXPECT_TRUE(sink.contains("\"msg\":\"agent.register\""));           // controller 가 등록 처리
    EXPECT_TRUE(sink.contains("\"msg\":\"agent.session.registered\"")); // agent 가 응답 수신
}
...
TEST(AgentControllerE2eTest, HeartbeatKeepsAgentAliveOverTime) {
    ...
    ASSERT_TRUE(pump_until(*controller, *agent, [&] { return agent->session().state() == State::active; }));
...
// operator API: controller.set_mode 로 c->a Command 발신 -> agent 적용 -> CommandOutcome 왕복.
TEST(AgentControllerE2eTest, OperatorSetModeRoundTrips) {
    ...
    bool const round_trip =
        pump_until(*controller, *agent, [&] { return sink.contains("\"msg\":\"command.outcome\""); });

    EXPECT_TRUE(round_trip);                                             // controller 가 outcome 수신/상관
    EXPECT_TRUE(sink.contains("\"msg\":\"agent.session.cmd.applied\"")); // agent 가 명령 적용
}

// controller 가 사라지면 agent 가 backoff 후 같은 포트의 새 controller 로 자동 재접속/재등록.
TEST(AgentControllerE2eTest, AgentReconnectsAfterControllerDrop) {
    ...
    bool const reactive =
        pump_until(*controller2, *agent, [&] { return agent->session().state() == State::active; }, 1500);
