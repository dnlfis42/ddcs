...
- **supersede 범위는 `(device, command_type)`입니다**: 같은 계열의 명령은 나중 것이 이전 것을 무효화하므로 계열당 미결 슬롯은 하나면 충분합니다.
  대체된 `command_id`로 오는 응답은 프로토콜 위반이 아니라 supersede의 정상적인 부산물이므로, `ddcs_command_stale_responses_total`로 집계만 하고 무시합니다.
...
|송신 큐 무제한|소비가 멈춘 Agent 하나가 Controller 메모리를 계속 차지. `ddcs_send_queue_messages`가 유일한 계기|상한 초과 시 연결 종료 (liveness와 동일한 결말)|
