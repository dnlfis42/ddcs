...
DDCS(Distributed Device Control System)는 분산 장치 제어 시스템이다. 단일 **Controller**가 다수의 **Agent**를 조율하고, 각 Agent는 정확히 하나의 **Device**를 호스팅한다(1 agent : 1 device : 1 session). 상태와 신원의 진실의 원천은 Agent 쪽 Device에 있고, Controller는 그 투영인 **DeviceShadow**만 보관한다. Controller는 각 Device를 Group 단위 **Policy**로 자동 조율하며, 사람이 직접 명령을 내리는 API는 없다 -- 명령은 정책 엔진이 발행한다.

이 문서는 시스템의 **구조와 동작을 개관**한다. 세부 권위는 다음 문서에 위임한다.

- 도메인 어휘(누가 무엇인가)는 [GLOSSARY.md](GLOSSARY.md)가 권위다.
- 와이어 프로토콜(바이트 레이아웃, 메시지 타입, 의미론)은 [PROTOCOL.md](PROTOCOL.md)가 권위다.
...
graph LR
  operator["operator"]
  prom["Prometheus"]
  subgraph Controller["Controller (단일 서버)"]
    core["세션 관리 + 정책 엔진"]
    shadows["DeviceShadow 저장소<br/>key = DeviceId"]
    metrics["HTTP metrics :9000"]
  end
  subgraph AgentA["Agent A (클라이언트)"]
    devA["Device A<br/>DeviceId A, 진실의 원천"]
  end
  subgraph AgentB["Agent B (클라이언트)"]
    devB["Device B<br/>DeviceId B, 진실의 원천"]
  end
  AgentA -- "Register / Status / Heartbeat" --> core
  AgentB -- "Register / Status / Heartbeat" --> core
  core -- "Command" --> AgentA
  core -- "Command" --> AgentB
  core -. "보고된 Status를 캐시" .-> shadows
  operator -. "controller.json 정책 주입" .-> core
  prom -- "scrape" --> metrics
```
...
| 모듈                      | 레이어      | 책임                                                                              |
| ------------------------- | ----------- | --------------------------------------------------------------------------------- |
| `common`                  | 기반        | 순수 C++ 값 타입(strong id, uuid, 버퍼, object pool, 시계, endian). POSIX 비의존. |
...
graph TD
  subgraph 실행["apps (실행)"]
    ctrl_exe["apps/ctrl"]
    agent_exe["apps/agent"]
  end
  subgraph controller["controller"]
    ctrl_facade["ctrl"]
    ctrl_app["ctrl::app"]
    ctrl_domain["ctrl::domain"]
    ctrl_infra["ctrl::infra"]
  end
  ...
  subgraph protocol["프로토콜 / 커널"]
    wire_frame["wire::frame"]
    wire_message["wire::message"]
    device["device"]
  end
  subgraph base["기반 / OS / 부가"]
    io["io"]
    net["net"]
    json["json"]
    logger["logger"]
  ...
  ctrl_exe --> ctrl_facade
  ctrl_exe --> config
  agent_exe --> agent_facade
  agent_exe --> agent_domain
  agent_exe --> config

  ctrl_facade --> ctrl_app
  ctrl_facade --> ctrl_infra
  ctrl_facade --> ctrl_domain
  ctrl_infra --> ctrl_app
  ctrl_infra --> io
  ctrl_infra --> net
  ctrl_infra --> wire_frame
  ctrl_app --> ctrl_domain
  ctrl_app --> wire_message
  ctrl_app --> device
  ctrl_app --> json
  ctrl_app --> logger
  ctrl_domain --> device
  ...
  agent_facade --> agent_app
  agent_facade --> agent_infra
  agent_facade --> agent_domain
  agent_infra --> agent_app
  agent_infra --> io
  agent_infra --> net
  agent_infra --> wire_frame
  agent_app --> agent_domain
  agent_app --> wire_message
  agent_app --> device
  agent_app --> json
  agent_app --> logger
  agent_domain --> device
...
각 측(`ctrl`/`agent`)은 동일하게 **domain -> app -> infra -> facade** 로 계층화된다.
...
> **왜 active만 집계/명령하나:** 끊긴 device의 stale Shadow load가 평균을 오염시키거나 미연결 대상에 헛 명령을 보내지 않도록, roster active 집합만 평균과 발신 대상으로 삼는다. 미보고 active device는 기본 `load=0`으로 들어간다.
...
- **재전송 = 동일 `command_id`** -- timeout/실패 시 지수 backoff 후 같은 id로 재전송하고, agent는 멱등하게 재실행한다. agent dedup은 depth-1(직전 `command_id`만 기억)이고, 중복이면 재적용 없이 캐시된 ack/outcome를 회신한다.
- **ACK-before-apply** -- agent는 디코드 성공 시 먼저 ack를 보내고, 그다음 적용 후 outcome을 보낸다. Controller는 ack로 "수신"과 "완료"를 분리해 느린 적용을 명령 유실로 오인하지 않는다.
...
> **왜 재전송이 동일 id + 멱등 재실행인가:** 현재 명령 어휘(`set_mode`)가 멱등 상태 선언이라 at-least-once 전달이 안전하다. **비멱등 명령 타입을 추가하려면** 그 타입은 수신측 dedup + outcome 재송신으로 격상해야 한다(현재 depth-1 캐시로는 부족). [PROTOCOL.md](PROTOCOL.md) 참고.
...
> **왜 jitter인가:** 다수 agent가 동시에 끊겼다 재연결할 때 재시도가 동기화되는 thundering-herd를 막으려 재시도 시점을 비상관화한다. cap(30s)은 복구된 Controller를 최악에도 ~30s 안에 재발견하도록 worst-case 지연을 bound한다.
