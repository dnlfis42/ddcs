...
public:
    ...
    [[nodiscard]] TimerHandler* consume(TimerToken id) noexcept {
    ...
    [[nodiscard]] bool erase(TimerToken id) noexcept {
        Slot* slot = resolve_slot(id);
        if (slot == nullptr) {
            return false;
        }
...
private:
    ...
    [[nodiscard]] Slot* resolve_slot(TimerToken id) noexcept {
        return const_cast<Slot*>(static_cast<TimerRegistry const*>(this)->resolve_slot(id));
    }

    [[nodiscard]] Slot const* resolve_slot(TimerToken id) const noexcept {
        if (!id.valid()) {
            return nullptr;
        }
    ...
    [[nodiscard]] static constexpr TimerToken
    pack(std::uint32_t generation, std::uint32_t index) noexcept {
        return TimerToken{(static_cast<std::uint64_t>(generation) << 32) | index};
    }

    [[nodiscard]] static constexpr std::uint32_t unpack_index(TimerToken id) noexcept {
        return static_cast<std::uint32_t>(id.get());
    }

    [[nodiscard]] static constexpr std::uint32_t unpack_generation(TimerToken id) noexcept {
