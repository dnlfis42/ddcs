...
//
// fd 디스패치는 HandlerTable(gen-token)로 일원화 - epoll data.u64 에 토큰을 싣고 resolve 로 되찾는다.
...
private:
    ...
    // signalfd 를 IoHandler 로 일원화 (table 경유 디스패치).
    struct SignalPump : IoHandler {
        Reactor* r{nullptr};
    ...
    HandlerTable handlers_; // fd -> IoHandler* (+gen). epoll data.u64 토큰의 출처/해석
