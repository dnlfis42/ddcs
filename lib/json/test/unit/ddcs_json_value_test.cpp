...
TEST(JsonValueTest, ParsesScalars) {
    ...
    EXPECT_EQ(parse("0")->as_int(), std::optional<std::int64_t>{0});
    EXPECT_EQ(parse("-0")->as_int(), std::optional<std::int64_t>{0});
    EXPECT_EQ(parse("  42 ")->as_int(), std::optional<std::int64_t>{42}); // 주변 공백
    EXPECT_EQ(parse("-7")->as_int(), std::optional<std::int64_t>{-7});
...
TEST(JsonValueTest, ParseDistinguishesIntAndDouble) {
    EXPECT_TRUE(parse("42")->as_int().has_value()); // 정수
    EXPECT_EQ(parse("42")->as_double(), std::optional<double>{42.0});
    EXPECT_FALSE(parse("42.0")->as_int().has_value()); // '.' 있으면 double
...
TEST(JsonValueTest, ParsesWhitespaceInContainers) {
    ...
    EXPECT_EQ(a->at(2)->as_int(), std::optional<std::int64_t>{3});
}
