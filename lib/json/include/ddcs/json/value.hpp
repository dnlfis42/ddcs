...
// object 는 삽입 순서를 보존한다(vector<pair> - 키 5~10개 규모라 선형 검색이 단순/빠름).
...
public:
    ...
    Value(int n) noexcept;           // -> int64 (int 리터럴이 int64/double 사이 모호해지지 않게)
    Value(std::int64_t n) noexcept;  // 정수
    Value(std::uint64_t n) noexcept; // -> int64 (캐스트; 우리 도메인 범위 안전)
    Value(double d) noexcept;        // 실수
    Value(char const* s);            // -> string
    ...
    // 타입 안전 접근 - 타입 불일치 시 nullopt(비-throw). as_string 의 view 는 이 Value 의 수명에 묶임.
    std::optional<bool> as_bool() const noexcept;
    std::optional<std::int64_t> as_int() const noexcept; // int64 일 때만
    std::optional<double> as_double() const noexcept;    // int64|double -> double (정수도 허용)
    std::optional<std::string_view> as_string() const noexcept;

    // object 연산 (삽입순 보존). null 이면 set 시 object 로 승격.
    ...
    // (동적 키 object 파싱용 - find 로는 키 목록을 모름.)
    ...
    // array 연산. null 이면 push_back 시 array 로 승격.
    ...
    static std::optional<Value> parse(std::string_view); // 실패(malformed/trailing/과도한 중첩) -> nullopt
