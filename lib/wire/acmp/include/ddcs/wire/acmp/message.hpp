...
inline constexpr std::size_t string_limit{std::numeric_limits<std::uint16_t>::max()};

enum class MessageType : std::uint8_t {
    invalid = 0x00,          // 유효하지 않은 값
    register_request = 0x01, // Agent -> Controller 등록 요청
    register_outcome = 0x02, // Controller -> Agent 등록 처리 결과
    register_ack = 0x03,     // Agent -> Controller 결과 수신 확인
    heartbeat = 0x10,        // Agent -> Controller keepalive
    status = 0x11,           // Agent -> Controller 상태 보고
    command_request = 0x20,  // Controller -> Agent 명령
    command_ack = 0x21,      // Agent -> Controller 명령 수신 확인
    command_outcome = 0x22,  // Agent -> Controller 명령 수행 결과
};
...
[[nodiscard]] std::optional<std::size_t> encode_command_ack(std::uint64_t command_id, std::span<std::byte> out) noexcept;
