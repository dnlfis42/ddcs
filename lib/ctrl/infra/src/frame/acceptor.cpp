...
bool Acceptor::init() noexcept {
    ...
    LOG_INFO("frame.acceptor.init", logger::kv("port", port()));
    return true;
}
...
bool Acceptor::reject_pending_connection(int exhausted_err) {
    LOG_WARN("frame.accept.fd_exhausted", logger::kv("errno", exhausted_err));
    ...
    if (!accept_spare_fd_.valid()) {
        ...
        LOG_ERROR("frame.accept.spare_fd_reopen_failed", logger::kv("errno", reopen_err));
