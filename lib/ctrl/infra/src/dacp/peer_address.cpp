...
constexpr std::size_t peer_address_format_max_len = 1 + INET6_ADDRSTRLEN + 1 + 1 + 5;
static_assert(
    peer_address_format_min_size >= peer_address_format_max_len,
    "peer_address_format_min_size must accommodate worst-case format"
);

} // namespace

std::string_view PeerAddress::format(std::span<char> buf) const noexcept {
    if (family == Family::none || buf.size() < peer_address_format_min_size) {
        return {};
    }
