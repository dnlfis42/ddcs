...
// 요청 완료 판정/응답 빌드/reap은 Server가 구동(정책 없음). HTTP read 후 respond 후 close
...
public:
    ...
    // 풀에서 꺼내 자원 배정 (idle에서 reading으로). fd 무효/이미 사용 중이면 false
    ...
    void begin_response(std::string http);                // 완성된 HTTP 응답 설정 후 writing
...
private: // io::ChannelHandler
    void
    on_ready(io::Channel& channel, io::ChannelEvents events) override; // 정책 없음, Server로 위임
