...
// 세션 수명 상태.
//  idle        : 미사용 초깃값.
//  handshaking : 연결됨, RegisterRequest 대기. 한도는 coordinator 의 handshake 타이머(infra)가 감시.
//  active      : 등록 완료, 운영 중. LivenessMonitor 가 last_seen 으로 침묵을 감시.
//  closing     : graceful close 진행 중. liveness 대상 아님(coordinator pw 가 드레인 한도).
...
struct Session {
    ...
    // active 트래픽 도착 -> liveness 갱신. (Dispatcher 가 active recv 마다 호출)
