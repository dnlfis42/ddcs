...
public:
    ...
    ConnectionId bind(ConnectionId conn, AgentId agent, common::Clock::time_point now);

    // agent -> 현재 conn (없으면 ConnectionId{}).
    ConnectionId resolve(AgentId agent) const;
...
private:
    ...
    std::unordered_map<AgentId, ConnectionId> agent_to_conn_; // 현재 바인딩
};
