...
// inbound 라우터. dacp 메시지를 decode해 wire 어휘를 use-case 어휘로 번역한다.
// - 연결 수명: on_connected -> AgentRegistry add, on_disconnected -> erase
// - 등록 3-way: RegisterRequest -> enroll + kick-old + bind + RegisterOutcome 송신, RegisterAck -> confirm
// - active: Heartbeat/Status/CommandAck/CommandOutcome를 의미 동사로 위임. 정상 처리된 메시지만 update_seen
...
public:
    ...
    void on_message(port::ConnectionId conn, std::uint8_t message_type, port::MessageBuffer body) override;
    void on_disconnected(port::ConnectionId conn, port::DisconnectReason reason) override;

private:
    void
    handle_register_request(port::ConnectionId conn, std::span<std::byte const> body, common::Clock::time_point now);
    void handle_register_ack(Agent& agent, std::span<std::byte const> body, common::Clock::time_point now);
    void handle_active_message(
        Agent& agent, std::uint8_t message_type, std::span<std::byte const> body, common::Clock::time_point now
    );

    bool send_register_outcome(port::ConnectionId conn, bool success, std::string_view reason);
    void kick(port::ConnectionId conn, std::string_view why); // 프로토콜 위반 -> 즉시 종료
