...
TEST(SessionLivenessMonitorTest, SilentActiveEvicted) {
    MonitorFixture f;
...
TEST(SessionLivenessMonitorTest, RecentlySeenActiveSurvives) {
    MonitorFixture f;
    ConnectionId const id = f.activate(1, 0xAA);
...
TEST(SessionLivenessMonitorTest, ExactTimeoutBoundarySurvives) {
    MonitorFixture f;
    f.activate(1, 0xAA);
...
TEST(SessionLivenessMonitorTest, HandshakingAgentIgnored) {
    MonitorFixture f;
...
TEST(SessionLivenessMonitorTest, ConfirmingAgentIgnored) {
    MonitorFixture f;
    ASSERT_TRUE(f.registry.add(ConnectionId{1}, f.clock.now()));
...
TEST(SessionLivenessMonitorTest, SweepSurvivesSynchronousErase) {
