...
TEST(SessionRegistryTest, BindActivatesAndResolves) {
    ...
    ConnectionId const kicked = reg.bind(ConnectionId{1}, AgentId{42}, {});
    EXPECT_FALSE(kicked.valid()); // 첫 바인딩 -> kick 없음

    auto* s = reg.find(ConnectionId{1});
    ASSERT_NE(s, nullptr);
    EXPECT_EQ(s->state, State::active);
    EXPECT_EQ(s->agent, AgentId{42});
    EXPECT_EQ(reg.resolve(AgentId{42}), ConnectionId{1});
}

TEST(SessionRegistryTest, BindKicksOldConnectionForSameAgent) {
    SessionRegistry reg;
    reg.open(ConnectionId{1});
    reg.bind(ConnectionId{1}, AgentId{7}, {});
    reg.open(ConnectionId{2});
    ConnectionId const kicked = reg.bind(ConnectionId{2}, AgentId{7}, {}); // 같은 agent, 새 conn

    EXPECT_EQ(kicked, ConnectionId{1});                  // 옛 conn 반환(kick 대상)
    EXPECT_EQ(reg.resolve(AgentId{7}), ConnectionId{2}); // 현재 바인딩 = 새 conn
}

TEST(SessionRegistryTest, RebindSameConnDoesNotKick) {
    SessionRegistry reg;
    reg.open(ConnectionId{1});
    reg.bind(ConnectionId{1}, AgentId{3}, {});
    ConnectionId const kicked = reg.bind(ConnectionId{1}, AgentId{3}, {}); // 같은 conn 재바인딩
    EXPECT_FALSE(kicked.valid());
    EXPECT_EQ(reg.resolve(AgentId{3}), ConnectionId{1});
}

TEST(SessionRegistryTest, EraseRemovesSessionAndReverse) {
    SessionRegistry reg;
    reg.open(ConnectionId{1});
    reg.bind(ConnectionId{1}, AgentId{5}, {});
    reg.erase(ConnectionId{1});
    EXPECT_EQ(reg.size(), 0u);
    EXPECT_FALSE(reg.resolve(AgentId{5}).valid()); // reverse 정리됨
...
TEST(SessionRegistryTest, EraseKickedOldConnPreservesReverse) {
    SessionRegistry reg;
    reg.open(ConnectionId{1});
    reg.bind(ConnectionId{1}, AgentId{9}, {});
    reg.open(ConnectionId{2});
    reg.bind(ConnectionId{2}, AgentId{9}, {}); // kick conn1 -> 현재 = conn2
    reg.erase(ConnectionId{1});                // 옛 conn disconnect

    EXPECT_EQ(reg.resolve(AgentId{9}), ConnectionId{2}); // reverse 는 conn2 가 가졌으므로 보존
