...
using ddcs::ctrl::app::agent::DeviceRoster;
...
TEST(DeviceRosterTest, YieldsOnlyActiveDevices) {
    ...
    DeviceRoster roster{agents};

    ASSERT_TRUE(agents.add(ConnectionId{1}, clock.now())); // handshaking
    ASSERT_TRUE(agents.add(ConnectionId{2}, clock.now()));
    ASSERT_TRUE(agents.bind(ConnectionId{2}, make_device_id(0xBB), clock.now())); // confirming
    ASSERT_TRUE(agents.add(ConnectionId{3}, clock.now()));
    ASSERT_TRUE(agents.bind(ConnectionId{3}, make_device_id(0xCC), clock.now()));
    ASSERT_TRUE(agents.find(ConnectionId{3})->confirm(clock.now())); // active
