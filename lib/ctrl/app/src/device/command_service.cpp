...
CommandService::CommandService(
    ...
    : sender_{sender}, command_timeout_{command_timeout}, max_attempts_{max_attempts}, backoff_base_{backoff_base} {}
...
port::CommandId CommandService::dispatch(
    ...
    auto const old_slot =
        std::find_if(device_commands.slots.begin(), device_commands.slots.end(), [command_type](Slot const& slot) {
            return slot.type == command_type;
        });
    if (old_slot != device_commands.slots.end()) {
        ++superseded_total_;
        LOG_INFO(
            "command.superseded", logger::kv("command", old_slot->id.value()), logger::kv("device", device.to_string())
        );
    ...
    ++dispatched_total_;
    LOG_INFO("command.dispatch", logger::kv("device", device.to_string()), logger::kv("command", command_id.value()));
    return command_id;
}

void CommandService::acknowledge(domain::DeviceId device, port::CommandId command_id, common::Clock::time_point now) {
...
void CommandService::sweep(common::Clock::time_point now) {
    ...
    for (auto const& [device, command_id] : due) {
        ...
        if (slot->phase == Phase::in_flight) {
            ++timed_out_total_;
            LOG_WARN(
                "command.timeout", logger::kv("command", command_id.value()), logger::kv("device", device.to_string())
            );
...
void CommandService::fail_attempt(domain::DeviceId device, port::CommandId command_id, common::Clock::time_point now) {
    Slot* const slot = find_slot(device, command_id);
    if (slot == nullptr) {
        return;
    }
    if (slot->attempts >= max_attempts_) {
        ++gave_up_total_;
        LOG_WARN(
            "command.gave_up", logger::kv("command", command_id.value()), logger::kv("device", device.to_string()),
            logger::kv("attempts", slot->attempts)
        );
...
void CommandService::resend(domain::DeviceId device, port::CommandId command_id, common::Clock::time_point now) {
    Slot* const slot = find_slot(device, command_id);
    if (slot == nullptr) {
        return;
    }
    ...
    LOG_INFO(
        "command.retry", logger::kv("command", command_id.value()), logger::kv("device", device.to_string()),
        logger::kv("attempt", slot->attempts)
    );
}
...
CommandService::Slot* CommandService::find_slot(domain::DeviceId device, port::CommandId command_id) {
...
void CommandService::close_slot(domain::DeviceId device, port::CommandId command_id) {
    ...
    std::erase_if(it->second.slots, [command_id](Slot const& slot) { return slot.id == command_id; });
